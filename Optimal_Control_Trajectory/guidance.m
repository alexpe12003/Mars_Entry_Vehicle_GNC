...
% === Integrate forward s_dot to get downrange ===
s_integrated = cumtrapz(t, s_dot_star);
s_star = s_integrated;
...
D_star  = DLS(:, 1);
L_star  = DLS(:, 2);
Cd      = simOut.Cd.signals.values;
Cl      = simOut.Cl.signals.values;
...
gain_table = compute_F1_F2_F3_from_costates( ...
    ...
    Mars_radius, m_0, Cd, Cl, rho, gs, Sref, sigma, L_star);

% === Bin velocity and average corresponding values ===
v_bin_width = 10;  % m/s
v_binned = round(v_star / v_bin_width) * v_bin_width;
[v_unique_ds, ~, bin_idx] = unique(v_binned);

% Average signals per bin
...
F1_ds       = accumarray(bin_idx, gain_table.F1, [], @mean);
F2_ds       = accumarray(bin_idx, gain_table.F2, [], @mean);
F3_ds       = accumarray(bin_idx, gain_table.F3, [], @mean);
